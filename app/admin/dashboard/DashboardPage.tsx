...
export default function DashboardPage({ data, error }: DashboardPageProps) {
    ...
    // data StatCard user 
    const dataProducts = data?.adminProductOverview
    // data StatCard order 
    const dataOrder  = data?.adminOrderOverview 
    // const dữ liệu doanh thu 
    const dataRevenue = data?.adminRevenueOverview
    // Chuan hoa chieu rong thanh bar theo doanh thu lon nhat trong tuan.
    const revenues = dataRevenue?.adminRevenueIn7day.map((item) => item.revenueInDay) ?? [];
    const maxRevenue = Math.max(...revenues, 0);
    // các ngày trong tuần cùng doanh thu 
    const RevenueIn7day = dataRevenue?.adminRevenueIn7day;
    // thông tin 5 order mới nhất
    const latestOrders = data?.adminOrderOverview.adminNewOrderOverview;
        ...
        <section>
            ...
            <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
                <StatCard
                    ...
                    note={
                        dataUser
                            ? `${dataUser.newUserIn7day} users mới trong 7 ngày`
                            : "_"
                    }
                    title="Tong users"
                    tone="blue"
                    value={dataUser ? `${dataUser.totalUser}`:"_"}
                />
                <StatCard
                    icon={<Box className="size-5" />}
                    note={dataProducts? `${dataProducts.productActive} san pham dc active` :"_"}
                    title="Tong products"
                    tone="violet"
                    value={dataProducts ? `${dataProducts.totalProducts}`:"_"}
                />
                <StatCard
                    icon={<ShoppingCart className="size-5" />}
                    note={dataOrder ? `${dataOrder.pendingOrderCount} đơn hàng chưa xác nhận`:"_"}
                    title="Don hang hom nay"
                    tone="amber"
                    value={dataOrder ?`${dataOrder.todayOrderCount}`:"_"}
                />
                <StatCard
                    icon={<CircleDollarSign className="size-5" />}
                    note={
                        dataRevenue
                            ? `${dataRevenue.weeklyRevenueGrowthRate > 0 ? "+" : ""}${dataRevenue.weeklyRevenueGrowthRate
                            }% so với tuần trước`
                            : "_"
                    }
                    title="Doanh thu tuan"
                    tone="emerald"
                    value={dataRevenue ? `${formatCompactCurrency(dataRevenue.weeklyRevenue)}`:"_"}
                />
            </div>
                    ...
                    <div className="flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <h2 className="mt-1 text-2xl font-bold text-slate-950">
                                {dataRevenue ? formatCurrency(dataRevenue.weeklyRevenue) : "_"}
                            </h2>
                        </div>
                        {dataRevenue ? (
                            <span className="w-fit rounded-md bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-700">
                                {dataRevenue.weeklyRevenueGrowthRate > 0 ? "+" : ""}
                                {dataRevenue.weeklyRevenueGrowthRate}% so voi tuan truoc
                            </span>
                        ) : null}
                    </div>
                    <div className="mt-4 space-y-3">
                        {RevenueIn7day?.length ? (
                            RevenueIn7day.map((item,index) => (
                                <div className="grid grid-cols-[118px_1fr_auto] items-center gap-3" key={item.createdAt}>
                                    <div className="flex items-baseline gap-2">
                                        <p className="text-sm font-medium text-slate-600">{dayInWeek[index]}</p>
                                        ...
                                        <div className="h-3 rounded-full bg-slate-100">
                                            <div
                                                ...
                                                style={{
                                                    width: `${maxRevenue > 0 ? Math.round((item.revenueInDay / maxRevenue) * 100) : 0}%`,
                                                }}
                                            />
                                        </div>
                            ...
                            {latestOrders?.length ? (
                                latestOrders.map((order) => (
                                    <tr className="border-b border-slate-100" key={order.id}>
                                        ...
                                        <td className="py-3 text-slate-600">{formatLocalDateTime(order.createdAt)}</td>
